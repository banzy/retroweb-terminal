...
let lastUnlockGestureAt = 0;
const pendingAfterUnlock: Array<(ac: AudioContext, out: GainNode) => void> = [];
...
function flushPendingAfterUnlock() {
  ...
  const callbacks = pendingAfterUnlock.splice(0);
  for (const cb of callbacks) cb(ctx, masterGain);
}
...
function unlockAudioContext() {
  ...
  if (ac.state !== "suspended" || unlockPromise) return;
  ...
  try {
    unlockPromise = ac
      ...
      .catch(() => {
        // Browsers can reject resume until a stricter user gesture arrives.
      })
      .finally(() => {
        unlockPromise = null;
      });
...
function hasRecentUnlockGesture(): boolean {
  if (typeof window === "undefined") return false;
  const nav = window.navigator as Navigator & {
    userActivation?: { isActive?: boolean };
  };
  if (nav.userActivation?.isActive) return true;
  return window.performance.now() - lastUnlockGestureAt < 1000;
}

function runWithAudio(cb: (ac: AudioContext, out: GainNode) => void) {
  const ac = ensureCtx();
  ...
  if (hasRecentUnlockGesture()) {
    pendingAfterUnlock.push(cb);
  }
  unlockAudioContext();
}
...
export function initAudioContext() {
  ...
  const tryUnlock = () => {
    lastUnlockGestureAt = window.performance.now();
    unlockAudioContext();
    if (audioReady) {
